...
pub enum Renderer {
    ...
    SpriteRenderer { img: Texture2D , scale: f32, tint: Color },
}

impl Renderer {
    pub fn image(path: &str, scale: f32, tint: Color, rl: &mut RaylibHandle, rlth: &RaylibThread) -> Renderer{
        Renderer::SpriteRenderer{img: rl.load_texture_from_image(rlth, &Image::load_image(path).unwrap()).unwrap(), scale, tint: tint}
    }
